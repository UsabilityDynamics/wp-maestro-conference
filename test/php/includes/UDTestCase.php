...
class UD_Plugin_WP_UnitTestCase extends WP_UnitTestCase {
  ...
  function setUp() {
    ...
    if( file_exists( $this->root_dir . '/wp-maestro-conference.php' ) ) {
      include_once( $this->root_dir . '/wp-maestro-conference.php' );
    }
    if( !class_exists( '\UsabilityDynamics\WP_MC\Bootstrap' ) ) {
      $this->fail( 'Plugin is not available.' );
    }
    $this->instance = \UsabilityDynamics\WP_MC\Bootstrap::get_instance();
  }
